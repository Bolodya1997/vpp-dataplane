...
func (s *Server) AddVppInterface(podSpec *storage.LocalPodSpec, doHostSideConf bool) (swIfIndex uint32, err error) {
	...
	for idx, hostPort := range podSpec.HostPorts {
		for _, containerAddr := range podSpec.ContainerIps {
			...
			id, err := s.BindHostPort(hostPort, containerAddr.IP)
			podSpec.HostPorts[idx].EntryID = id
			if err != nil {
				return 0, s.tunErrorCleanup(podSpec, err, "error binding hostport")
			}
		}
	}
...
func (s *Server) DelVppInterface(podSpec *storage.LocalPodSpec) error {
	...
	if ok {
		for _, hostport := range initialSpec.HostPorts {
			...
			if err != nil {
				return errors.Wrapf(err, "(del) Error deleting entry with ID %s", hostport.EntryID)
			}
